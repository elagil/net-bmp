...
/**
 * @file
 * @brief   The GDB command module headers.
 *
 * @addtogroup gdb
 * @{
 */
...
#define GDB_COMMAND_MAX_ARG_COUNT 32u

#define GDB_COMMAND_REPLY_EMPTY          ""    // An empty response, usually for unsupported requests.
#define GDB_COMMAND_REPLY_OK             "OK"  // A reply for successful execution of a command.
#define GDB_COMMAND_REPLY_CONSOLE_OUTPUT "O"   // Leads a reply that results in GDB console output.

// The indices of error replies are not used by the GDB client - they can be chosen arbitrarily.
#define GDB_COMMAND_REPLY_ERROR_01 "E01"  // Error reply with index 1
#define GDB_COMMAND_REPLY_ERROR_02 "E02"  // Error reply with index 2
#define GDB_COMMAND_REPLY_ERROR_03 "E03"  // Error reply with index 3

enum gdb_command_result {
    GDB_COMMAND_RESULT_OK,
    GDB_COMMAND_RESULT_EXCESS_ARGUMENTS,
};
 ...
 */
static inline void gdb_command_reply(struct gdb_session* p_gdb_session, char* reply) {
