...
/**
 * @file
 * @brief   The USB configuration module.
 *
 * @addtogroup usb_cfg
 * @{
 */
...
SerialUSBDriver g_serial_usb_driver;
...
static const uint8_t vcom_configuration_descriptor_data[67] = {
    ...
    USB_DESC_BCD(0x0110),     // bcdCDC. Call Management Functional Descriptor.
    USB_DESC_BYTE(5),         // bFunctionLength.
    USB_DESC_BYTE(0x24),      // bDescriptorType (CS_INTERFACE).
    USB_DESC_BYTE(0x01),      // bDescriptorSubtype (Call Management Functional Descriptor).
    USB_DESC_BYTE(0x00),      // bmCapabilities (D0+D1).
    USB_DESC_BYTE(0x01),      // bDataInterface. ACM Functional Descriptor.
    USB_DESC_BYTE(4),         // bFunctionLength.
    USB_DESC_BYTE(0x24),      // bDescriptorType (CS_INTERFACE).
    USB_DESC_BYTE(0x02),      // bDescriptorSubtype (Abstract Control Management Descriptor).
    USB_DESC_BYTE(0x02),      // bmCapabilities. Union Functional Descriptor.
    USB_DESC_BYTE(5),         // bFunctionLength.
    USB_DESC_BYTE(0x24),      // bDescriptorType (CS_INTERFACE).
    ...
    USB_DESC_ENDPOINT(USB1_INTERRUPT_REQUEST_EP | 0x80, 0x03,  // bmAttributes (Interrupt).
...
static const uint8_t vcom_string2[] = {USB_DESC_BYTE(56),                     // bLength.
                                       USB_DESC_BYTE(USB_DESCRIPTOR_STRING),  // bDescriptorType.
                                       'C',
                                       0,
                                       'h',
...
static const USBDescriptor vcom_strings[] = {{sizeof vcom_string0, vcom_string0},
...
/**
 * @brief Handles the GET_DESCRIPTOR callback.All required descriptors must be handled here.
 *
 * @param usbp A pointer to the USB driver structure.
 * @param dtype The descriptor type.
 * @param dindex The descriptor index.
 * @param lang The language identifier.
 * @return const USBDescriptor* A pointer to the USB descriptor that was requested.
 */
...
static void usb_event(USBDriver *usbp, usbevent_t event) {
    switch (event) {
        ...
        case USB_EVENT_CONFIGURED:
            ...
            // Enables the endpoints specified into the configuration. Note, this callback is invoked from an ISR so I -
            // Class functions must be used.
            usbInitEndpointI(usbp, USB1_DATA_REQUEST_EP, &ep1config);
            usbInitEndpointI(usbp, USB1_INTERRUPT_REQUEST_EP, &ep2config);

            // Resetting the state of the CDC subsystem.
            sduConfigureHookI(&g_serial_usb_driver);
        ...
        case USB_EVENT_SUSPEND:
            ...
            sduSuspendHookI(&g_serial_usb_driver);
        ...
        case USB_EVENT_WAKEUP:
            ...
            sduWakeupHookI(&g_serial_usb_driver);
...
/**
 * @brief Handles the USB start-of-frame package.
 *
 * @param usbp A pointer to the USB driver structure.
 */
static void sof_handler(USBDriver *usbp) {
    ...
    osalSysLockFromISR();
    sduSOFHookI(&g_serial_usb_driver);
...
void usbcfg_init(void) {
    sduObjectInit(&g_serial_usb_driver);
    sduStart(&g_serial_usb_driver, &serusbcfg);
...
/**
 * @brief Get the serial USB-CDC stream.
 * @return BaseSequentialStream * The pointer to the stream that the USB-CDC driver provides.
 */
BaseSequentialStream *get_serial_usb_stream(void) { return (BaseSequentialStream *)&g_serial_usb_driver; }
